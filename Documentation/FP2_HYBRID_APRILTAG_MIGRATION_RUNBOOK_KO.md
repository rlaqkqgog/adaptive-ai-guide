...
- 태그가 붙은 벽을 따른 고정 기준 `-0.65 m` + 기준 조정 `+0.15 m` = 최종 `-0.50 m`
- 태그 벽에서 방 안쪽으로 `0.25 m`
- 높이 추가 조정 `0.00 m`

이 값은 태그 회전으로 만든 수평 기준축에 적용되므로 태그/방의 yaw가 바뀌어도
월드 X/Z를 직접 추측하지 않는다. 단, 이 `-0.50/0.25 m`는 **FP1 Room3에서 얻은
현장 고유값**이다. FP2에서 숫자를 그대로 복사하지 말고 FP2 기준 벽에서 다시
측정한다.

2026-07-25 현장 재검토에서 벽을 따른 이동을 `0.50 m`에서 `0.20 m`로 줄였다.
따라서 기존 적용 위치에서 같은 기준축을 따라 `0.30 m` 되돌린 상태다.
벽 이격도 `0.40 m`에서 `0.25 m`로 줄여 벽 쪽으로 `0.15 m` 되돌렸다.
이후 벽을 따른 이동은 `0.65 m`로 다시 조정했으며, 현장 확인 후 최종 방향을
반전하여 `-0.65 m`로 기준 위치를 복원한 뒤, 이 고정 기준에서 `+0.15 m`를 조정해
최종 `-0.50 m`로 확정했다. 이전 시험값 `-0.55 m`는 계산에 사용하지 않는다.
...
3. Apply 성공 후 상세 AprilTag 정렬 HUD가 자동으로 사라지고, `A` Play 이후에도
   다시 나타나지 않는지 확인한다.
4. 머리를 좌우·상하로 움직여 큐브가 벽에 붙어 보이고 시점과 함께 떠다니지 않는지 확인한다.
5. 콘텐츠가 전체적으로 벽을 따라 앞/뒤로 틀렸으면 `alongWallBackMeters`만 조정한다.
6. 콘텐츠가 태그 벽에 너무 붙었거나 멀면 `wallClearanceMeters`만 조정한다.
7. 높이는 위 두 값으로 바꾸지 않는다. 높이 오류는 태그 기준점/수직 보정 문제로 분리한다.
8. 한 번에 한 축, 0.10–0.20 m 단위로 바꾸고 새 APK에서 재확인한다.
